...
(async () => {
  ...
  app.get("/cars/", async (req: Request, res: Response) => {
    ...
    if (make) {
      ...
      console.log("no make was provided");
  ...
  app.get("/cars/:id", async (req: Request, res: Response) => {
    const { id } = req.params;
    let requiredCar = cars.find((car) => car.id === Number(id));
    if (requiredCar) {
      return res.status(200).send(requiredCar);
    }
    return res.status(404).send("no car was found with this id");
  });

  /// @TODO Add an endpoint to post a new car to our list
  // it should require id, type, model, and cost
  app.post("/cars/", async (req: Request, res: Response) => {
    const { name, id, type, model, make, cost } = req.body;
    // if (!name) {
    ...
    let newCar = { id, make, cost, type, model };
    cars.push(newCar);
